...
def train_test_split(X, y, test_size=0.0, shuffle=True, random_state=None):
    ...
    if shuffle:
        ...
        Xy_pair = pd.concat([pd.DataFrame(X), pd.DataFrame(y)], axis=1, join='outer', ignore_index=False)
        # Now shuffle.
        Xy_pair = Xy_pair.reindex(np.random.RandomState(random_state).permutation(Xy_pair.index))
        # Finally, seperate the dataframe back to X and y parts.
        y = pd.DataFrame(Xy_pair.T.iloc[-1])
        X = pd.DataFrame(Xy_pair.iloc[:, :-1])

    # Now split the dataframes.
    # X_train split.
    X_train_split = X[0: len(y) - first_split_size]  # .head(len(y) - first_split_size)
    # X_test split.
    X_test_split = X[len(y) - first_split_size:]  # .tail(first_split_size)
    # y_train split.
    y_train_split = pd.DataFrame(y[0: len(y) - first_split_size]).T
    y_train_split = y_train_split[y_train_split.columns].values
    y_train_split = y_train_split[0]
    # y_test split.
    y_test_split = pd.DataFrame(y[len(y) - first_split_size:]).T
